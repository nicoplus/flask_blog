...
from . import main
from .forms import CommentForm, NameForm, EditProfileForm, EditProfileAdminForm, PostForm
from .. import db
...
def edit_profile_admin(id):
	...
	if form.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()
		flash('The progile has been updated')
...
def post(id):
	...
	if form.validate_on_submit():
		...
		db.session.commit()
		flash('You have add the comment.')
		return redirect(url_for('.post', id=post.id, page=-1))

	page = request.args.get('page', 1, type=int)
	if page==-1:
		page = (Post.query.count()-1) / current_app.config['FLASKY_COMMENTS_PER_PAGE'] + 1
...
def edit(id):
	...
	if form.validate_on_submit():
		...
		flash('You have update the post.')
...
def unfollow(username):
	...
	if not current_user.is_following(user):
		flash('You have cancled follow.')
		return redirect(url_for('.user', username=username))
	current_user.unfollow(user)
	flash('You are unfollowing.')
	return redirect(url_for('.user', username=username))
...
def followers(username):
	...
	return render_template('followers.html', user=user, follows=follows, title='Followers of', endpoint='.followers', pagination=pagination)
...
def followed_by(username):
	...
	return render_template('followed.html', user=user, follows=follows, title='Followed by', endpoint='.followed_by', pagination=pagination)
